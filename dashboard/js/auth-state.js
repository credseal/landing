/**
 * CredSeal Auth State Management
...
(function(global) {
  ...
  class AuthState {
    ...
    _setUser(user) {
      this._user = user;
      localStorage.setItem('credseal_user', JSON.stringify(user));
    }
    ...
    getUser() {
      if (!this._user) {
        const stored = localStorage.getItem('credseal_user');
        if (stored) {
          try {
    ...
    _setOrg(org) {
      this._org = org;
      localStorage.setItem('credseal_org', JSON.stringify(org));
    }
    ...
    getOrg() {
      if (!this._org) {
        const stored = localStorage.getItem('credseal_org');
        if (stored) {
          try {
    ...
    _cleanup() {
      ...
      // Clear stored data
      localStorage.removeItem('credseal_access_token');
      localStorage.removeItem('credseal_user');
      localStorage.removeItem('credseal_org');
    }
    ...
    _onStorageChange(event) {
      if (event.key === 'credseal_access_token') {
        if (!event.newValue) {

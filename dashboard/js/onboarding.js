...
(function() {
  ...
  const STORAGE_KEY = 'credseal_onboarding_completed';
  const STORAGE_STEP_KEY = 'credseal_onboarding_step';
  ...
  const steps = [
    ...
    },
    {
      ...
      description: 'Copy this code snippet to start using CredSeal in your agent.',
      content: `
      ...
      onEnter: () => {
        ...
        document.getElementById('onboarding-copy-btn')?.addEventListener('click', async () => {
          ...
          if (code) {
            ...
            window.CredSeal?.showToast?.('Code copied to clipboard', 'success');
          }
        });
      }
    }
  ...
  function updateCodeSnippet(lang, credName, agentId) {
    ...
    const snippets = {
      python: `from credseal import CredSeal

# Initialize the client
cs = CredSeal(agent_id="${agentId}")
...
with cs.session("${credName}") as session:
    ...
    response = requests.get(
      ...
      typescript: `import { CredSeal } from '@credseal/sdk';

// Initialize the client
const cs = new CredSeal({ agentId: '${agentId}' });
...
try {
  ...
  function next() {
    ...
    if (step.validate) {
      ...
      if (error) {
        window.CredSeal?.showToast?.(error, 'warning');
        return;
      }
    }
  ...
  function skip() {
    ...
    close();
    window.CredSeal?.showToast?.('You can always access the quickstart guide later', 'info');
  }
  ...
  function complete() {
    ...
    close();
    window.CredSeal?.showToast?.('Setup complete! Welcome to CredSeal', 'success');
